...
\label{sec:motivation}
	We would like to improve development of web applications with ASP.NET Web Forms and therefore it is beneficial to investigate how JavaScript can be safely represented in order to achieve compile time validation. % Todo: synes denne første del er lidt underlig...
...
\section{Reading Guide}
	...
	% Todo: The read should have knowledge of DSL, JavaScript (script), client server side
...
\section{Scope}
	...
	\label{sub:safety_considerations}
		...
		\label{ssub:server_client_consistency}
			When writing scripts from server side (Figure \ref{buttonScript}) a consistency problem can occur. Script code that calls a function has no guarantee that this specific call is actually valid. One could have misspelled the function name or passed too few arguments. Guaranteed consistency between such calls (Server Client Side Consistency) would improve correctness and is therefore a safety consideration we have included in this project.
