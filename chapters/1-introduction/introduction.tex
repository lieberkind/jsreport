...
\label{ssub:compile_time_errors_vs_runtime_errors}
	Compile time errors are easier to debug than runtime errors and therefore also less time consuming. When considering web applications errors can occur both on server and client side. Client side runtime errors will be exposed to web application end users which is undesirable in most situations and therefore another reason for preventing client side runtime errors. Because of this one of the fundamental purposes of our safe JavaScript project is to try and move errors from client side runtime to server side (runtime or compile time) as illustrated in Figure TODO: REF.
